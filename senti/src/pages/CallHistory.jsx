    ...
    <div style={{ display: 'flex', alignItems: 'center' }}>
      ...
      >
        <Box sx={{ ...style, width: 300, borderRadius: '10px', height: 300 }} className="relative">
          ...
          <div className='absolute top-40 right-16 flex flex-col items-center'>
            <div className=' mb-1'>OR</div>
            <div className=' flex'>

              <select ref={clientref} className='py-1 px-2 rounded-l-full border-0 min-w-[100px] text-center'>
                {clients.map((e) => (<option className=' outline-none' value={e.id}>{e.username}</option>))}
              </select>
              <button
                type="button"
                style={{ color: '#03C9D7', backgroundColor: '#E5FAFB' }}
                className="text-md opacity-0.9 p-2 hover:drop-shadow-xl  rounded-r-full"
